...
class TestCreateMcpServer:
    ...
    def test_create_mcp_server_default(self):
        ...
        assert mcp.name == "FastAPI + FastMCP Starter"
...
class TestCreateApp:
    ...
    def test_create_app_with_auth(self):
        """Test creating FastAPI app with authentication."""
        app = create_app(auth_token="test-token")
        client = TestClient(app)

        # Health should be accessible without auth (excluded)
        response = client.get("/health")
        assert response.status_code == 200
    ...
    def test_create_app_mcp_endpoint(self):
    ...
    def test_create_app_custom_mcp(self):
